...
public class WebReactiveHttpClient<P extends Publisher<?>> implements ReactiveHttpClient<P> {
	private final Logger log = LoggerFactory.getLogger(WebReactiveHttpClient.class);
...
//		request.headers().get("Content-Type").stream().findFirst().get();

		log.warn("Request Header {}", request.headers());
		log.warn("Request Body {}", request.body());
		...
		if(request.body() instanceof SerializedFormData){
			...
			log.warn("MULTIPART  {}", ((PublisherClientMethodHandler.MultipartMap) request.body()).getMap());
